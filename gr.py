...
class Render(object):
    ...
    def transform(self, vertex, vMatrix):
        ...
        transVertex = op.mtxmul(vMatrix , augVertex)
    ...
    def createModelMatrix(self, translate = (0,0,0), scale = (1,1,1), rotate=(0,0,0)):
        ...
        return op.mtxmul(translateMatrix,op.mtxmul( rotationMatrix , scaleMatrix))

    def createRotationMatrix(self, rotate=(0,0,0)):

        pitch = op.deg2rad(rotate[0])
        yaw = op.deg2rad(rotate[1])
        roll = op.deg2rad(rotate[2])
        ...
        return op.mtxmul(rotationX,op.mtxmul( rotationY , rotationZ))
    ...
    def BgImage(self, path):
        background = Obj.Texture(path)
        for x in range(background.height):
            for y in range(background.width):
                self.glVertex_coord(y, x, background.pixels[x][y])
